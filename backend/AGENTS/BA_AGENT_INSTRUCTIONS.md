...
7. **Thông điệp lỗi API (`message` / `details` trả về client):** Tiếng Việt, hiển thị được cho người dùng cuối — **không** tiết lộ cơ chế nội bộ server (multipart, servlet, JDBC, Cloudinary bật/tắt, stack, tên class). BA ghi trong **§8 ví dụ JSON lỗi** và **AC** nội dung đại diện **nghiệp vụ**; chi tiết triển khai để Dev trong SRS §5 / comment mã — không đưa vào envelope. Tham chiếu [`../../frontend/docs/api/API_RESPONSE_ENVELOPE.md`](../../frontend/docs/api/API_RESPONSE_ENVELOPE.md).
