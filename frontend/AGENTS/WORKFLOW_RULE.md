...
- **Cách “tự động” khả thi**: trong **một phiên chat**, AI đọc file này rồi **đóng vai tuần tự** BA → PM → TECH_LEAD → DEV → CODEBASE_ANALYST → DOC_SYNC, báo rõ từng bước.  
...
| G4 | DEV | CODEBASE_ANALYST | Test pass, **coverage ≥ 80%**, `lint` + `build` pass (theo [`DEVELOPER_AGENT_INSTRUCTIONS.md`](DEVELOPER_AGENT_INSTRUCTIONS.md)) |
| G5 | CODEBASE_ANALYST | DOC_SYNC | Báo cáo 10-phase đã xuất (path trong mục 5) |
| G6 | DOC_SYNC | Kết thúc | Báo cáo drift đã xuất; PM cập nhật trạng thái SRS / PM_RUN nếu có |
...
- Một lệnh orchestrate (một phiên):  
  `WORKFLOW_RULE: chạy full sau Gate Owner (PM → … → DOC_SYNC) cho SRS …`
